...
contract Polymarket {
    ...
    constructor(address _paraToken) {
        owner = msg.sender;
    ...
    function addYesBet(uint256 _questionId, uint256 _value) public payable {
        ...
        require(
            block.timestamp < questions[_questionId].endTimestamp,
            "Betting period has ended"
        );
        require(
            !questions[_questionId].eventCompleted,
            "Event is already completed"
        );
        Questions storage question = questions[_questionId];
        ERC20(paraToken).transferFrom(msg.sender, address(this), _value);
    ...
    function distributeWinningAmount(
        ...
        if (eventOutcome) {
            ...
            for (uint256 i = 0; i < winningAddresses.length; i++) {
                ...
                ERC20(paraToken).transfer(_address, winningAmount[_address]);
